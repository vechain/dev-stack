...
export async function removeVolume(name) {
  try {
    await run('docker', ['volume', 'rm', name], { capture: true })
  } catch {
    // volume may not exist or may still be in use; not fatal
  }
}

// Env vars that must be present for docker-compose variable substitution.
// The thor-* vars all have defaults in compose/base.yaml so they're passed
// through verbatim — only set them here if the user has already set them.
...
export async function composeStop(files, services) {
  const env = await composeRequiresEnv()
  const args = [...composeArgs(files), 'stop', ...services]
  await run('docker', args, { env })
}

export async function composeLogs(files, services, { follow = false } = {}) {
  const env = await composeRequiresEnv()
  const args = [...composeArgs(files), 'logs']
