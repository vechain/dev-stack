...
- `vechain-dev down` only stops the **consumer's overlay** services (if `overlay:` is set). Shared infra stays running on purpose so the next project can use it. Use `vechain-dev reset` to actually tear everything down and wipe `~/.vechain-dev/`.
